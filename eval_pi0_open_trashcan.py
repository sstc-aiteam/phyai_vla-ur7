#!/usr/bin/env python
"""Open-loop evaluation of a trained pi0 policy against recorded ground-truth
actions in the open_trashcan dataset.

Mirrors `eval_smolvla_open_trashcan.py` (same open-loop replay/MAE
methodology, same held-out-episode protocol) but loads a `PI0Policy`
checkpoint. Must run inside the sibling pi0 venv set up by
`scripts/setup_pi0_env.sh` (this repo's own env can't import
lerobot.policies.pi0 -- see that script's docstring for why):
    ...
    uv run --project ../lerobot-pi0 python eval_pi0_open_trashcan.py \\
        ...
        --output outputs/sample_size_sweep/pi0_open_trashcan_100_eval.json
"""
...
def main():
    ...
    parser.add_argument("--checkpoint", default="outputs/pi0_open_trashcan/checkpoints/last/pretrained_model")
    ...
    parser.add_argument("--output", default="outputs/pi0_open_trashcan_eval.json")
