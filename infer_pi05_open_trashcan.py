#!/usr/bin/env python
"""Closed-loop pi05 inference on the real UR7e arm.

Mirrors `infer_smolvla_open_trashcan.py`/`infer_act_open_trashcan.py` exactly
(same physical motion pattern: blocking moveJ to the first pose, then a
servoJ stream at the recording fps) -- the only difference is loading a
`PI05Policy` checkpoint instead of `SmolVLAPolicy`/`ACTPolicy`. See
`infer_act_open_trashcan.py`'s docstring for the full motion-pattern
rationale.

Unlike `infer_groot_open_trashcan.py`, this checkpoint was trained with
`use_relative_actions=false` (see `scripts/train_pi0.sh` -- it doesn't set
`--policy.use_relative_actions`, so it's off by default), so `PI05Policy`'s
plain `select_action()` action-queue behavior works fine here -- no chunk-
decoding workaround needed. See that script's docstring for why relative
actions would have required one.

The checkpoint's own saved preprocessor (`policy_preprocessor.json`) already
embeds the `--rename_map` used at training time
(`observation.images.cam_wrist` -> `observation.images.left_wrist_0_rgb`),
so this script feeds observations under the original `cam_wrist` key --
same as `eval_pi05_open_trashcan.py` -- and the loaded preprocessor renames
them internally before they reach the model.

ENVIRONMENT -- this needs BOTH pi05 (matching `../lerobot-pi0`'s schema --
checkpoints trained against its checkout use newer `lerobot` internals this
repo's own pinned env doesn't have, see `scripts/setup_pi0_env.sh`'s
docstring for why it's a separate transformers version -- `../lerobot-pi0`
already has pi05 available too, it's the same lerobot checkout, see
`scripts/run_pi05_sweep.sh`) *and* real-time UR robot control (`ur-rtde`,
`pyrealsense2`) in the same process. Neither `../lerobot-pi0` (missing the
robot packages) nor this repo's own env (missing the `pi`/`training`
extras) has both, so run this inside the same dedicated conda env used for
pi0 inference, `pi0_infer` (mirrors `groot_infer` -- see README.md; already
has pi05 available for the same reason as `../lerobot-pi0` above, so no
separate env is needed):

    conda create -n pi0_infer python=3.12 -y   # skip if it already exists (e.g. from pi0 inference)
    ...
    python infer_pi05_open_trashcan.py --checkpoint ... --robot-ip ...
...
SAFETY -- read before running:
  ...
  * pi05 predicts chunk_size=50-step action chunks and queues them
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--checkpoint",
                         default="outputs/sweep_170/pi05_open_trashcan_full/checkpoints/last/pretrained_model",
                         help="Default is the sample-size sweep's best pi05 checkpoint (full 140-episode "
                              "training pool, lowest held-out MAE -- see scripts/run_sweep_170_eval100_129.sh). "
                              "Other sizes: outputs/sweep_170/pi05_open_trashcan_{10,25,50,100}/checkpoints/last/pretrained_model")
    ...
    parser.add_argument("--cam-wrist-index", type=int, default=None,
                         help="Camera index for the wrist view -- must match how open_trashcan was "
    ...
    parser.add_argument("--cam-wrist-backend", default="realsense", choices=("usb", "realsense", "remote"),
                         help="'remote' reads from a camera_server.py process over TCP instead of a "
                              "locally attached camera (e.g. one streamed from a Jetson mounted on the "
                              "arm) -- see --cam-wrist-host/--cam-wrist-port")
