#!/usr/bin/env python
"""Closed-loop pi0 inference on the real UR7e arm.
...
`PI0Policy` checkpoint instead of `SmolVLAPolicy`/`ACTPolicy`. See
...
`--policy.use_relative_actions`, so it's off by default), so `PI0Policy`'s
...
same as `eval_pi0_open_trashcan.py` -- and the loaded preprocessor renames
them internally before they reach the model.

ENVIRONMENT -- this needs BOTH pi0 (matching `../lerobot-pi0`'s schema --
...
docstring for why it's a separate transformers version) *and* real-time UR
robot control (`ur-rtde`, `pyrealsense2`) in the same process. Neither
`../lerobot-pi0` (missing the robot packages) nor this repo's own env
(missing the `pi`/`training` extras) has both, so run this inside a
dedicated conda env, e.g. `pi0_infer` (mirrors `groot_infer` -- see
README.md):
    ...
    python infer_pi0_open_trashcan.py --checkpoint ... --robot-ip ...
...
SAFETY -- read before running:
  ...
  * pi0 predicts chunk_size=50-step action chunks and queues them
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--checkpoint", default="outputs/pi0_open_trashcan_100/checkpoints/last/pretrained_model")
...
def main():
    ...
    policy = PI0Policy.from_pretrained(ckpt, device=args.device)
