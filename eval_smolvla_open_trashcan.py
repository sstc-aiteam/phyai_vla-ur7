#!/usr/bin/env python
"""Open-loop evaluation of a trained SmolVLA policy against recorded ground-truth
actions in the open_trashcan dataset.

Mirrors `eval_act_open_trashcan.py` (same open-loop replay/MAE methodology)
but loads a `SmolVLAPolicy` checkpoint instead of `ACTPolicy` -- see that
script's docstring for the general approach.
"""
...
def main():
    ...
    parser.add_argument("--checkpoint", default="outputs/smolvla_open_trashcan/checkpoints/last/pretrained_model")
    ...
    parser.add_argument("--output", default="outputs/smolvla_open_trashcan_eval.json")
